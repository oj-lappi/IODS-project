...
For this, I've created a shellscript called "knit", which is in this repo, now we just need to write a git hook which checks if index.Rmd is newer than index.html, and runs knit if so, which we can do using a Makefile.

Now, the git hook, which looks like this:
