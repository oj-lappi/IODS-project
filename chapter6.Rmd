...
```{r fig.align="center", fig.cap = "Linear model", fig.width = 16, fig.height = 16, results = 'hide'}
plot(bprs.lm, which=1)
```
...
In a random intercept model, the per-subject random effect is modeled as a normal distribution centered on zero and variance $\hat{\sigma}^2_u$, but is this valid if the intercept distribution is skewed?
Seemingly so, after a quick search.

**Should we standardize the bprs scores?**
...
The deviance and AIC have both dropped, which tells us that adding the slope gives us a much better fit.
...
```{r fig.align="center", fig.cap = "Linear model", fig.width = 16, fig.height = 9, results = 'hide'}
